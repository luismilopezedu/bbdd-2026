...
# Ejemplo - Redirigir a una URL concreta según el rol al realizar el login 

En uno de los ejemplos anteriores, teníamos el siguiente código en la clase `SecurityConfig`:
...
Tal y como vimos en el ejemplo de seguridad con `UserDetailsService`, la interfaz `UserDetails` define un método que nos devuelve una colección de `GrantedAuthority`, y que habitualmente, se definen como uno o varios roles para el usuario (con el prefijo `ROLE_`, por ejemplo `ROLE_ADMIN`).
